...
impl State {
        ...
        match message {
            ...
            Event::AddAccountModal(message) => {
                return self
                    .add_account_modal
                    .update(message, |jid| {
                        self.accounts.insert(jid.clone().into(), Account::new(jid.clone(), Status::Online));
                    })
            ...
            Event::BackendEvent(backend_event) => {
                match backend_event {
                    ...
                    BackendEvent::Contact { account, contact, chats } => {
                        todo!()
                        //self.contacts
                        //    .get_mut(&account)
                        //    .unwrap()
            ...
            Event::NewMessageSubmit => {
                ...
                self.sender.send_blocking(UiEvent::NewText {
                    from,
                    to,
    ...
    pub fn view(&self) -> Element<Event> {
        ...
        let menut = Menu::new(menu_items!(
            (button("sssss").width(200))
        ...
        let account_pannel = 
            container(
                                ...
                                button(
                                    ...
                                    .style(move |theme, status| {
                                        if let Some(cur_acc) = &current_acc {
                                            ...
                                            return button::Style {
                                                background: Some(Background::Color(Color::from_rgb8(100, 100, 0))),
                                                ..Default::default()
                                            };
                                        }
                                    };
                ...
                container::Style {
                    border: Border { color: Color::from_rgb8(0, 255, 0), width: 1.0, radius: Radius::new(1) },
                    ..Default::default()
                }
            });
        ...
        if let Some(current_acc) = &self.current_account {
                ...
                for (jid, _contact) in contacts {
                    ...
                    let button = Element::new(
                        ...
                        .style(move |_theme, _state| {
                            ...
                            let mut background = Some(Background::Color(Color::from_rgba8(200, 200, 200, 0.2)));

                            if let Some(cur_con) = &cur_contact {
                                if cur_con == jid {
                                    background = Some(Background::Color(Color::from_rgba8(200, 200, 200, 0.8)));
                                }
                            };
                            ...
                            button::Style {
                                ...
                                background,
                                border: Border { color: Color::TRANSPARENT, width: 0.0, radius: Radius::new(10.0) },
                                ..Default::default()
                            }
                        })
                    );
            ...
            contact_column = contact_column.push(button("+").on_press(AddContactModalMessage::Open.into()));
        ...
        if let Some(current_contact) = &self.current_contact {
            ...
            for msg in &contact.chat_history {
                ...
                let element = Element::new(
                    container(
                        column![
                            text(content)
                            .align_x(if msg.by_me { Horizontal::Left } else { Horizontal::Right }).width(Fill),

                            container(text(format!("{}", msg.stamp)))
                            //.width(Length::Fill)
                            .style(|theme| {
                                container::Style { 
                                    border: Border { color: Color::from_rgb8(255, 0, 0), width: 1.0, ..Default::default() }, 
                                    ..Default::default()
                                }
                            }),
                        ]
            ...
            if let Some(acc) = &self.current_account {
                if let Some(acc) = self.accounts.get(acc) {
                    match acc.status() {
                        Status::Offline => {
                            msg_pannel = msg_pannel.push(text("You need to be online to send messages..."));
                            disable_message_box = true;
                        }
                        Status::Connecting => {
                            msg_pannel = msg_pannel.push(text("still connecting. you can message when you're online"));
                            disable_message_box = true;
                        }
                        _ => {},
                    }
            ...
            msg_pannel = msg_pannel.push(
                ...
                .on_input_maybe(if disable_message_box { None } else { Some(Event::NewMessageTextInput) })
