...
use iced::{window, Background, Border, Color, Point, Task};
    ...
    fn new(
        ...
        (
            ...
            Task::batch([
                ...
                Task::run(reciever, |event| Message::Dashboard(Event::BackendEvent(event))),
            ])
        )
    }
    ...
    pub fn update(&mut self, message: Message) -> Task<Message> {
        ...
        match message {
            Message::Dashboard(message) => {
                return self.dashboard.update(message).map(|message| Message::Dashboard(message));
            }
        }
