...
use tokio_xmpp::{
    jid::Jid, Client, Event as TokioXmppEvent, Stanza
};
use chrono::Utc;
use log::info;
use xmpp_parsers::presence::{Presence, Show as PresenceShow, Type as PresenceType};
use async_channel::{Sender, Receiver};
use crate::core::{contact::Contact, event::{BackendEvent, UiEvent}};
...
pub async fn run(
    ...
    for (jid, password) in accounts {
        ...
        sender.send(BackendEvent::Account(jid.clone(), Account::new((**jid).clone(), Status::Connecting))).await.unwrap();
        ...
        for contact in contacts {
            let chats = db::get_messages(&jid, &contact)
                .await
                .into_iter()
                .map(|(by_me, body, timestmap)| {
                    Message::new_text(body, by_me, timestmap)
                }).collect();

            //subscribe(Jid::from((*contact).clone()));

            contactsss.push((jid.clone(), contact.clone(), Contact::new(contact.clone(), Status::Offline, chats))); 
        }
        
        if contactsss.is_empty() == false {
            sender.send(BackendEvent::Contacts(contactsss)).await.unwrap();
        }
    }
...
async fn online(
    ...
    sinks.get_mut(&jid).unwrap().send(presence.into()).await.unwrap();
    sender.send(BackendEvent::AccountOnline(jid.clone(), bound_jid)).await.unwrap(); 
...
fn stream_to_jid_stream(
    stream: SplitStream<XmppTlsClient>, jid: AccountId
...
fn handle_payloads(payloads: Vec<Element>, id: String) {
    ...
    for elm in payloads {
        match elm.name() {
            "request" => {
                let elm = Element::builder("recieved", "urn:xmpp:reciepts")
                    .attr("id", id.clone());
