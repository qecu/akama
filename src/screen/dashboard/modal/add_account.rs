...
impl AddAccount {
    ...
    pub fn view<'a>(&self) -> Element<'a, Message> {
        let content = container(
            column![
                text("Add Account"),
                text_input("jid", &self.text_input_jid).on_input_maybe(if self.submit_pending {
                    None
                } else {
                    Some(Message::TextInputJid)
                }),
                text_input("password", &self.text_input_password).on_input_maybe(
                    if self.submit_pending {
                        None
                    } else {
                        Some(Message::TextInputPassword)
                    }
                ),
                text(self.error.clone()),
                button("Sumbit").on_press_maybe(if self.submit_pending {
                    None
                } else {
                    Some(Message::Submit)
                }),
...
async fn new_xmpp_client(
    ...
    match event {
        ...
        tokio_xmpp::Event::Disconnected(err) => Err(err.to_string()),
        _ => {
